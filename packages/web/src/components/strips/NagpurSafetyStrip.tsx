...
const SAFETY_CATEGORIES = ['crime'];
const MAX_ITEMS = 8;

export function NagpurSafetyStrip() {
  ...
  const items = SAFETY_CATEGORIES.flatMap((cat) => digest?.categories?.[cat] ?? [])
    .sort((a, b) => new Date(b.publishedAt).getTime() - new Date(a.publishedAt).getTime())
    .slice(0, MAX_ITEMS);

  if (items.length === 0) {
    return <p className="text-sm text-gray-400 py-2 text-center">{t('panel.nagpurSafety.empty')}</p>;
  }
  ...
  return (
    <>
      ...
      <TileFooter stale={isStale}>
        {t('panel.nagpurSafety.source')}{agoText ? ` · ${agoText}` : ''}
      </TileFooter>
    </>
  );
}
