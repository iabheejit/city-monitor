...
import { useNmcAnnouncements } from '../../hooks/useCivic.js';
...
export function NmcAnnouncementsStrip() {
  const { id: cityId } = useCityConfig();
  const { data, fetchedAt, isLoading } = useNmcAnnouncements(cityId);
  ...
  if (items.length === 0) {
    return <p className="text-sm text-gray-400 py-2 text-center">{t('panel.nmcAnnouncements.empty')}</p>;
  }
  ...
  return (
    <>
      ...
      <TileFooter stale={isStale}>
        {t('panel.nmcAnnouncements.source')}{agoText ? ` · ${agoText}` : ''}
      </TileFooter>
    </>
  );
}
